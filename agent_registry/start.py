...
from agent_registry.config import CONN_TIMEOUT, TLS_CIPHTER, FORWARDED_ALLOW_IPS
...
class CustomUvicornServer:
    ...
    def run(self):
        ...
        server_config = uvicorn.config(
            ...
            ssl_certfile=self.conf_obj.ssl.certfile,
            # 私钥路径
            ssl_keyfile=self.conf_obj.ssl.keyfile,
            # 私钥密码
            ...
            ssl_ciphers=CipherConverter.convert(self.server_config.get(TLS_CIPHTER)),
...
def main():
    ...
    try:
        # 校验配置
        config_obj = conf_singleton_obj
        result = CertValidator(config_obj).validate()
        if not result.is_valid:
            sys.exit(result.message)
        # 通过校验后修改etc/ssl文件夹权限为700，里面文件权限为600
        set_ssl_folder_permissions()
        # 创建并启动服务器
        server = CustomUvicornServer(server_config, config_obj)
        server.run()
