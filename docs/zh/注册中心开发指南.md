...
flowchart TD
    subgraph 注册流程
        R1[HandlerRegistry.register<br>interface_type, handler_class] --> R2[存储到注册表字典]
    end

    subgraph 查找流程
        G1[HandlerRegistry.get_handler<br>interface_type] --> G2{查找注册表}
        G2 -->|找到| G3[返回自定义处理器实例]
        G2 -->|未找到| G4[返回默认处理器实例]
    end
    ...
    subgraph 类继承结构
        direction TB
        B[BaseHandler 抽象基类<br>定义 handle 抽象方法]
        C[自定义处理器<br>继承 BaseHandler]
        B -.->|子类必须实现| C
        C --> I[实现 async def handle<br>*args, **kwargs]
    end
