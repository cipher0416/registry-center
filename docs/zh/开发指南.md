...
**步骤1**：在 `common/custom` 目录下新增 `__init__.py` 和 `my_custom_handle.py` 文件。

**步骤2**：在 `my_custom_handle.py` 文件中创建自定义处理器
...
> ⚠️ **注意**：如果同一个接口注册多个自定义处理器，后注册的会覆盖前面注册的处理器。请确保注册顺序符合预期。
